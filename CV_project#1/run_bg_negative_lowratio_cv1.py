...
RUNS_DIR = ROOT / "bg_neg_low_runs"
DS_ROOT = ROOT / "datasets_bgneg_low"

TARGET = ["person", "car", "dog"]
CLS2IDX = {c: i for i, c in enumerate(TARGET)}

NEG_RATIOS = [0.1, 0.2, 0.3]
SEEDS = [111, 222, 333]
...
def build_dataset_with_negatives(ratio: float, neg_pool: dict):
    ...
    for sp in ["train", "val"]:
        ...
        rng = random.Random(5000 + int(ratio * 1000) + (0 if sp == "train" else 99))
...
def confusion_bg_counts(weights_path: Path):
    ...
    fp_bg_to_cls, fn_cls_to_bg, tp = 0, 0, 0
    for img_path in sorted((VOC3_ROOT / "images" / "val").glob("*.jpg")):
        ...
        for ln in lp.read_text(encoding="utf-8").splitlines():
            ...
            gts.append({"cls": ci, "box": [(cx - bw / 2) * w, (cy - bh / 2) * h, (cx + bw / 2) * w, (cy + bh / 2) * h]})
        ...
        for b in r.boxes:
            ...
            if name not in CLS2IDX:
                continue
            preds.append({"cls": CLS2IDX[name], "box": b.xyxy[0].cpu().numpy().tolist()})
...
def train_one(ratio: float, seed: int, ds_yaml: Path):
    tag = f"bgneglow_r{int(ratio*100):02d}_s{seed}"
    exp = RUNS_DIR / tag
    if exp.exists():
...
def summarize(rows):
    out = []
    for rr in sorted({float(r["ratio"]) for r in rows}):
...
def baseline_metrics():
    ...
    return {"map50": float(b["map50"]), "map50_95": float(b["map50_95"]), "precision": float(b["precision"]), "recall": float(b["recall"])}
...
def make_figures(sum_rows, base):
    ...
    plt.ylabel("score")
    plt.title("Low Negative Ratio: Performance")
    plt.legend()
    plt.tight_layout()
    p1 = FIG_DIR / "bgneg_low_performance_vs_ratio_cv1.png"
    ...
    plt.title("Low Negative Ratio: Background Error Trend")
    plt.legend()
    plt.tight_layout()
    p2 = FIG_DIR / "bgneg_low_fp_fn_vs_ratio_cv1.png"
...
def main():
    ...
    raw_csv = OUT_DIR / "bgneg_low_experiment_raw_cv1.csv"
    ...
    sum_csv = OUT_DIR / "bgneg_low_experiment_summary_cv1.csv"
    ...
    rep = []
    rep.append("# Low-Ratio Background Negative Experiment Report (CV1)")
    rep.append("")
    rep.append("- ratios tested: [0.1, 0.2, 0.3], seeds=[111,222,333]")
    rep.append("- fixed training recipe: epochs=16, imgsz=640, batch=12, lr0=0.0009, freeze=5, mosaic=0.6, mixup=0.1, close_mosaic=3")
    rep.append("")
    ...
    rep.append("")
    rep.append("![performance](" + fig1.as_posix() + ")")
    rep.append("")
    rep.append("![fp_fn](" + fig2.as_posix() + ")")
    rep.append("")
    rep.append(
        f"- best ratio={best['ratio']:.1f}: mAP50={best['map50_mean']:.4f}±{best['map50_std']:.4f}, "
        f"mAP50-95={best['map50_95_mean']:.4f}±{best['map50_95_std']:.4f}, "
        f"FP={best['fp_bg_to_cls_mean']:.2f}, FN={best['fn_cls_to_bg_mean']:.2f}"
    )
    rep.append(
    ...
    report_md = OUT_DIR / "bgneg_low_experiment_report_cv1.md"
    ...
    man = OUT_DIR / "bgneg_low_experiment_manifest_cv1.json"
